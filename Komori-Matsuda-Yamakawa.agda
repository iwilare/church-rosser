...
basetto : ∀{n m}{M M′ : Term n}{σ : Subst n m}
  → M —→ M′
    -----------------------
  → subst σ M —→ subst σ M′
basetto (—→-ξ₁ x) = —→-ξ₁ (basetto x)
basetto (—→-ξ₂ x) = —→-ξ₂ (basetto x)
basetto {σ = σ}(—→-β{N = N}{M = M}) rewrite sym (subst-commute{N = N}{M = M}{σ = σ}) = —→-β
basetto (—→-ƛ x) = —→-ƛ (basetto x)
...
subst-betas{σ = σ}{τ} s (L —→⟨ x ⟩ k) = subst σ L —→⟨ basetto x ⟩ subst-betas s k  -- subst σ L —→⟨ {!!} ⟩ {!subst-beta!} {- {M = # x₁} s (.(# x₁) ∎) = s
...
int : ∀{n} (a b : Term (suc n)) (c : Term n)
  → (a · b) [ c ] —↠ (a [ c ] · b [ c ])
int = λ a b c → subst (subst-zero c) a · subst (subst-zero c) b ∎
...
lemminatio : ∀{n}{s : Term n} → subst-zero (s *) ≡ ts (subst-zero s)
lemminatio = extensionality (λ{ Data.Fin.0F → refl ; (suc r) → refl })

lemma3-4 : ∀{n} (t : Term (suc n)) (s : Term n)
    → t * [ s * ] —↠ (t [ s ]) *
lemma3-4 t s rewrite lemminatio{s = s} = lemma-gen (subst-zero s) t

lemminazione : ∀{n}{L′ : Term (suc n)}{B : Term n}
    → (ƛ L′ *) · B * —↠ (L′ *) [ B * ]
lemminazione {L′ = L′}{B = B} = beta-complete (L′ * ∎) (B * ∎)
